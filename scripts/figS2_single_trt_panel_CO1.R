...
# Fig S2 -------------------------------------------------------------------
# single-treatment coi data
# upload two-trt cvs
pilot_single_trt <- readr::read_csv("data/species_by_sites_matrix_habitat_COI_orig_split.csv")
# remove trt column
pilot_single_trt$veg_score <- NULL

# make sure there are no zero-occurrence species
pilot_single_trt <- pilot_single_trt[,c(TRUE, colSums(pilot_single_trt[,-c(1)]) > 0 )]
...
ggsave(full2,
       filename = "figures/raw_pdfs/figS2_single_trt_panel_CO1.pdf",
       width = 12*.85,
       height = 6*.85,
       device = cairo_pdf)

# find species change for 363 species of log2 0.07
log2(381/363)
381 - 363
363 - 18
log2(346/363)
